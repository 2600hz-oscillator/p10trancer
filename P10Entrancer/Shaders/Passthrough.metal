...
// Simpler full-screen-quad preview for the output-pad row. No aspect
// scaling — just samples the source texture across the full quad.
vertex PVOut outputPreviewVertex(uint vid [[vertex_id]]) {
    float2 positions[3] = { float2(-1.0, -3.0), float2(-1.0, 1.0), float2(3.0, 1.0) };
    PVOut out;
    out.position = float4(positions[vid], 0.0, 1.0);
    out.uv = (positions[vid] + float2(1.0, 1.0)) * 0.5;
...
fragment half4 outputPreviewFragment(
    PVOut in [[stage_in]],
    texture2d<half> src [[texture(0)]]
) {
    constexpr sampler s(filter::linear, address::clamp_to_edge);
