...
private struct FeedbackEditor: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            slider("Decay",    $state.decay,       in: 0.85...0.99, format: "%.3f")
            slider("FB Mix",   $state.feedbackMix, in: 0...1,       format: "%.2f")
            slider("Luminosity", $state.luminosity,  in: 0...2, format: "%.2f×")
            slider("Chroma boost", $state.chromaBoost, in: 0...3, format: "%.2f×")
            Text("Tip: zoom > 1 with FB Mix > 0.7 produces the recursive tunnel. Zoom < 1 puts a black border around the source. Push Luminosity / Chroma if the tunnel goes dark or grey.")
